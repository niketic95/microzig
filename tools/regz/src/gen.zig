...
fn write_device_file(
    ...
    var ast = try std.zig.Ast.parse(arena, to_zero_sentinel(buf.written()), .{ .mode = .zig });
...
fn write_peripherals_files(io: std.Io, db: *Database, arena: Allocator, dir: std.Io.Dir, opts: ToZigOptions) !void {
    ...
    for (peripherals) |peripheral| {
        ...
        try writer.writeByte(0);
        var ast = try std.zig.Ast.parse(arena, to_zero_sentinel(periph_content.written()), .{ .mode = .zig });
        defer ast.deinit(arena);
